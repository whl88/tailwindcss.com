...
function prefixEnDocsPath(path: string): string {
  if (!path.startsWith("/docs")) return path;
  if (path.startsWith("/en/docs")) return path;
  const [pathname, ...hashParts] = path.split("#");
  const base = "/en" + pathname;
  return hashParts.length ? `${base}#${hashParts.join("#")}` : base;
}

/** Mirror /docs/* redirects for English locale URLs (/en/docs/*). */
function addEnglishDocRedirects(list: RedirectEntry[]): RedirectEntry[] {
  const extra: RedirectEntry[] = [];
  for (const r of list) {
    if (r.source.startsWith("/docs") && !r.source.startsWith("/en/docs")) {
      const dest =
        typeof r.destination === "string" && r.destination.startsWith("http")
          ? r.destination
          : prefixEnDocsPath(r.destination);
      extra.push({
        source: "/en" + r.source,
        destination: dest,
