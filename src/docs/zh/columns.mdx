...
export const description = "用于控制元素内列数的工具类。";
...
使用 `columns-3` 等 `columns-<number>` 工具类，设置元素内容应分成几列：
...
使用 `columns-xs`、`columns-sm` 等工具类，设置元素内容理想的列宽：
...
设置列宽后，列数会自动调整，以确保各列不会太窄。

### 设置列间距

使用 `gap-<width>` 工具类，指定列与列之间的间距：
