...
export const description = "用于控制元素边框宽度的工具类。";
...
使用 `border` 或 `border-2`、`border-4` 等 `border-<number>` 工具类，设置元素所有边的边框宽度：
...
使用 `border-r`、`border-t-4` 等工具类，单独设置元素某一侧的边框宽度：
...
使用 `border-x`、`border-y-4` 等工具类，同时设置元素相对两侧的边框宽度：
...
使用 `border-s`、`border-e-4` 等工具类，设置 `border-inline-start-width` 与 `border-inline-end-width` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，它们会根据文字方向映射到左侧或右侧的边框：
...
使用 `border-bs-<number>` 和 `border-be-<number>` 工具类，设置 `border-block-start-width` 与 `border-block-end-width` 逻辑属性，它们会根据书写模式映射到顶部或底部的边框：
...
使用 `divide-x`、`divide-y-4` 等工具类，在相邻子元素之间添加分隔线：
...
如果元素是反向排列的（例如使用了 `flex-row-reverse` 或 `flex-col-reverse`），请使用 `divide-x-reverse` 或 `divide-y-reverse` 工具类，确保分隔线被加到每个元素正确的一侧：
