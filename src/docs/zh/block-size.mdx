...
export const description = "用于设置元素 block 方向尺寸的工具类。";
...
使用 `block-24`、`block-64` 等 `block-<number>` 工具类，按 spacing 比例为元素设置固定的 block 尺寸：
...
使用 `block-full` 或 `block-1/2`、`block-2/5` 等 `block-<fraction>` 工具类，给元素设置基于百分比的 block 尺寸：
...
使用 `block-screen` 工具类，让元素的 block 尺寸撑满整个视口：
...
使用 `block-dvh` 工具类，让元素的 block 尺寸撑满整个视口（随浏览器 UI 展开/收起而变化）：
...
使用 `block-lvh` 工具类，把元素的 block 尺寸设为视口可能的最大值：
...
使用 `block-svh` 工具类，把元素的 block 尺寸设为视口可能的最小值：
