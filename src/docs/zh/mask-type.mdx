...
export const description = "用于控制 SVG mask 解读方式的工具类。";
...
使用 `mask-type-alpha` 和 `mask-type-luminance` 工具类，控制 SVG mask 的类型：
...
使用 `mask-type-luminance` 时，SVG mask 的亮度值决定可见度，因此使用灰度颜色能得到最可预测的结果。使用 `mask-alpha` 时，则由 SVG mask 的不透明度决定被遮罩元素的可见度。
