...
export const description = "用于控制单个 flex / grid 子项在交叉轴方向上的对齐方式的工具类。";
...
使用 `self-auto` 工具类，让单个子项按容器 `align-items` 的取值对齐：
...
使用 `self-start` 工具类，无视容器的 `align-items` 值，让单个子项在交叉轴上对齐到起始端：
...
使用 `self-center` 工具类，无视容器的 `align-items` 值，让单个子项在交叉轴上居中：
...
使用 `self-end` 工具类，无视容器的 `align-items` 值，让单个子项在交叉轴上对齐到末端：
...
使用 `self-stretch` 工具类，无视容器的 `align-items` 值，让单个子项沿交叉轴拉伸以填满容器：
...
使用 `self-baseline` 工具类，让单个子项的基线与 flex 容器交叉轴上的基线对齐：
...
使用 `self-baseline-last` 工具类，让单个子项的基线沿容器交叉轴与容器中最后一条基线对齐：
