...
export const description = "用于控制元素是否避开其前面浮动元素的工具类。";
...
使用 `clear-left` 工具类，把元素放置在之前所有左浮动元素的下方：
...
使用 `clear-right` 工具类，把元素放置在之前所有右浮动元素的下方：
...
使用 `clear-both` 工具类，把元素放置在之前所有浮动元素的下方：
...
使用 `clear-start` 和 `clear-end` 工具类，借助[逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) 根据文字方向映射到左侧或右侧：
...
使用 `clear-none` 工具类，移除元素上已设置的 clear：
