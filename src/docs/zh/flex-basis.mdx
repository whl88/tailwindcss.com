...
export const description = "用于控制 flex 子项初始尺寸的工具类。";
...
使用 `basis-64`、`basis-128` 等 `basis-<number>` 工具类，按 spacing 比例设置 flex 子项的初始尺寸：
...
使用 `basis-xs`、`basis-sm` 等工具类，按容器尺寸比例设置 flex 子项的初始尺寸：
...
使用 `basis-1/2`、`basis-2/3` 等 `basis-<fraction>` 工具类，按分数设置 flex 子项的初始尺寸：
