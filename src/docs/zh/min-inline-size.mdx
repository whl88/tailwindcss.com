...
export const description = "用于设置元素 inline 方向最小尺寸的工具类。";
...
使用 `min-inline-24`、`min-inline-64` 等 `min-inline-<number>` 工具类，按 spacing 比例为元素设置固定的最小 inline 尺寸：
...
使用 `min-inline-full` 或 `min-inline-1/2`、`min-inline-2/5` 等 `min-inline-<fraction>` 工具类，给元素设置基于百分比的最小 inline 尺寸：
...
使用 `min-inline-sm`、`min-inline-xl` 等工具类，按容器尺寸比例为元素设置固定的最小 inline 尺寸：
