...
export const description = "用于控制 flex / grid 子项沿容器主轴排列方式的工具类。";
...
使用 `justify-start` 工具类，让子项沿容器主轴对齐到起始端：
...
使用 `justify-center` 或 `justify-center-safe` 工具类，让子项沿容器主轴居中：
...
空间不足时，`justify-center-safe` 工具类会把子项对齐到容器的起始端，而不是居中。

### End 末端

使用 `justify-end` 或 `justify-end-safe` 工具类，让子项沿容器主轴对齐到末端：
...
空间不足时，`justify-end-safe` 工具类会把子项对齐到容器的起始端，而不是末端。

### Space between 等距分布

使用 `justify-between` 工具类，让子项沿容器主轴排列，相邻子项之间留出相等的间距：
...
使用 `justify-around` 工具类，让子项沿容器主轴排列，每个子项两侧都留出相等的间距：
...
使用 `justify-evenly` 工具类，让子项沿容器主轴排列，每个子项周围都留出相等的间距，且不会像 `justify-around` 那样在子项之间出现两倍间距：
...
使用 `justify-stretch` 工具类，让自动尺寸的子项沿容器主轴方向拉伸以填满可用空间：
...
使用 `justify-normal` 工具类，让子项按默认位置排列，等同于没有设置 `justify-content`：
