...
export const description = "用于控制元素内容溢出处理方式的工具类。";
...
使用 `overflow-visible` 工具类，避免元素内的内容被裁剪：
...
使用 `overflow-hidden` 工具类，把元素内超出其边界的内容裁掉：
...
使用 `overflow-auto` 工具类，在内容超出元素边界时为其添加滚动条：
...
使用 `overflow-x-scroll` 工具类，允许水平滚动并始终显示滚动条，除非操作系统禁用了常显滚动条：
...
使用 `overflow-y-scroll` 工具类，允许垂直滚动并始终显示滚动条，除非操作系统禁用了常显滚动条：
...
使用 `overflow-scroll` 工具类，给元素添加滚动条：
