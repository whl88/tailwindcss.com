...
export const description = "用于控制元素跨越的网格列以及尺寸的工具类。";
...
使用 `col-span-2`、`col-span-4` 等 `col-span-<number>` 工具类，让元素跨越 _n_ 列：
...
使用 `col-start-2`、`col-end-3` 等 `col-start-<number>` 或 `col-end-<number>` 工具类，让元素从第 _n_ 条网格线开始或结束：
...
它们还可以与 `col-span-<number>` 工具类组合，跨越指定数量的列。
