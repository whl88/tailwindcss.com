...
export const description = "用于平移元素的工具类。";
...
使用 `translate-2`、`-translate-4` 等 `translate-<number>` 工具类，按 spacing 比例尺在两个轴向同时位移元素：
...
使用 `translate-1/4`、`-translate-full` 等 `translate-<fraction>` 工具类，按元素自身尺寸的百分比在两个轴向同时位移元素：
...
使用 `translate-x-4`、`translate-x-1/4` 等 `translate-x-<number>` 或 `translate-x-<fraction>` 工具类，沿 x 轴位移元素：
...
使用 `translate-y-6`、`translate-y-1/3` 等 `translate-y-<number>` 或 `translate-y-<fraction>` 工具类，沿 y 轴位移元素：
...
使用 `translate-z-6`、`-translate-z-12` 等 `translate-z-<number>` 工具类，沿 z 轴位移元素：
