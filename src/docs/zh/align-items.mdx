...
export const description = "用于控制 flex / grid 子项在交叉轴方向上的对齐方式的工具类。";
...
使用 `items-stretch` 工具类，让所有子项沿交叉轴拉伸以填满容器：
...
使用 `items-start` 工具类，让所有子项在交叉轴上对齐到起始端：
...
使用 `items-center` 工具类，让所有子项在交叉轴上居中：
...
使用 `items-end` 工具类，让所有子项在交叉轴上对齐到末端：
...
使用 `items-baseline` 工具类，让所有子项的基线沿容器交叉轴对齐：
...
使用 `items-baseline-last` 工具类，让所有子项的基线沿容器交叉轴与容器中最后一条基线对齐：
