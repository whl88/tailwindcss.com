...
export const description = "用于控制元素 mask 模式的工具类。";
...
使用 `mask-alpha`、`mask-luminance` 和 `mask-match` 工具类，控制元素 mask 的模式：
...
使用 `mask-luminance` 时，mask 的亮度值决定可见度，因此使用灰度颜色能得到最可预测的结果。使用 `mask-alpha` 时，则由 mask 的不透明度决定被遮罩元素的可见度。
