...
export const description = "用于一次性控制 grid 子项横纵两轴对齐方式的工具类。";
...
使用 `place-items-start`，让 grid 子项在两个轴向都对齐到所在网格区域的起始端：
...
使用 `place-items-end`，让 grid 子项在两个轴向都对齐到所在网格区域的末端：
...
使用 `place-items-center`，让 grid 子项在两个轴向都对齐到所在网格区域的中心：
...
使用 `place-items-stretch`，让子项在两个轴向都沿网格区域拉伸：
