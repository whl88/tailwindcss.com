...
export const description = "用于控制元素圆角半径的工具类。";
...
使用 `rounded-sm`、`rounded-md` 等工具类，给元素设置不同尺寸的圆角：
...
使用 `rounded-t-md`、`rounded-r-lg` 等工具类，只给元素的某一侧添加圆角：
...
使用 `rounded-tr-md`、`rounded-tl-lg` 等工具类，只给元素的某一个角添加圆角：
...
使用 `rounded-s-md`、`rounded-se-xl` 等工具类，通过[逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)设置圆角，它们会根据文字方向映射到相应的角：
...
下面列出了所有圆角逻辑属性工具类，以及它们在 LTR 和 RTL 模式下对应的物理属性。
