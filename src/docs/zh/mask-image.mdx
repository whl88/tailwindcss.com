...
export const description = "用于控制元素 mask 图像的工具类。";
...
使用 `mask-[<value>]` 语法，设置元素的 mask 图像：
...
使用 `mask-b-from-<value>`、`mask-t-to-<value>` 等工具类，给元素的某一侧加上线性渐变 mask：
...
此外，使用 `mask-x-from-70%`、`mask-y-to-90%` 等工具类，可以同时给元素相对两侧加上 mask：
...
使用 `mask-linear-<angle>`、`mask-linear-from-20`、`mask-linear-to-40` 等工具类，为元素添加自定义的线性渐变 mask：
...
使用 `mask-radial-from-<value>` 和 `mask-radial-to-<value>` 工具类，为元素添加径向渐变 mask：
...
使用 `mask-radial-at-bottom-left`、`mask-radial-at-[35%_35%]` 等工具类，设置径向渐变 mask 的圆心位置：
...
这与 [`mask-position`](/docs/mask-position) 不同——后者控制的是 mask 图像本身的位置，而不是径向渐变的位置。

#### Setting the radial size

使用 `mask-radial-closest-corner`、`mask-radial-farthest-side` 等工具类，设置径向渐变 mask 的尺寸：
...
设置自定义径向渐变尺寸时，可用单位取决于渐变的 `<ending-shape>`，默认是 `ellipse`。
...
使用 `mask-conic-from-<value>`、`mask-conic-to-<value>` 和 `mask-conic-<angle>` 工具类，为元素添加锥形渐变 mask：
...
这一行为依赖于 Tailwind 默认把 [`mask-composite` 属性](/docs/mask-composite)设为 `intersect`。修改该属性会影响多个渐变 mask 的组合方式。

### 移除蒙版图像

使用 `mask-none` 工具类，移除元素已有的 mask 图像：
