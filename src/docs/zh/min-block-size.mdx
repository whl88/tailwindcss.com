...
export const description = "用于设置元素 block 方向最小尺寸的工具类。";
...
使用 `min-block-24`、`min-block-64` 等 `min-block-<number>` 工具类，按 spacing 比例为元素设置固定的最小 block 尺寸：
...
使用 `min-block-full` 或 `min-block-1/2`、`min-block-2/5` 等 `min-block-<fraction>` 工具类，给元素设置基于百分比的最小 block 尺寸：
