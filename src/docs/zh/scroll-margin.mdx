...
export const description = "用于控制 snap 容器中滚动停靠时元素外侧偏移的工具类。";
...
使用 `scroll-ml-4`、`scroll-mt-6` 等 `scroll-mt-<number>`、`scroll-mr-<number>`、`scroll-mb-<number>`、`scroll-ml-<number>` 工具类，为 snap 容器内各项设置滚动偏移：
...
如需使用负的 scroll margin 值，在类名前加上连字符即可将其转换为负值：
...
使用 `scroll-ms-<number>` 和 `scroll-me-<number>` 工具类，设置 `scroll-margin-inline-start` 与 `scroll-margin-inline-end` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，它们会根据文字方向映射到左侧或右侧：
...
使用 `scroll-mbs-<number>` 和 `scroll-mbe-<number>` 工具类，设置 `scroll-margin-block-start` 与 `scroll-margin-block-end` 逻辑属性，它们会根据书写模式映射到顶部或底部：
