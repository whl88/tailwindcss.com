...
export const description = "用于控制 snap 容器中滚动停靠时容器内侧偏移的工具类。";
...
使用 `scroll-pl-4`、`scroll-pt-6` 等 `scroll-pt-<number>`、`scroll-pr-<number>`、`scroll-pb-<number>`、`scroll-pl-<number>` 工具类，设置 snap 容器内元素的滚动偏移：
...
使用 `scroll-ps-<number>` 和 `scroll-pe-<number>` 工具类，设置 `scroll-padding-inline-start` 与 `scroll-padding-inline-end` 逻辑属性，它们会根据文字方向映射到左侧或右侧：
...
使用 `scroll-pbs-<number>` 和 `scroll-pbe-<number>` 工具类，设置 `scroll-padding-block-start` 与 `scroll-padding-block-end` 逻辑属性，它们会根据书写模式映射到顶部或底部：
...
如需使用负的 scroll padding 值，在类名前加上连字符即可将其转换为负值：
