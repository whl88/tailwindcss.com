...
export const description = "用于设置元素 inline 方向尺寸的工具类。";
...
使用 `inline-24`、`inline-64` 等 `inline-<number>` 工具类，按 spacing 比例为元素设置固定的 inline 尺寸：
...
使用 `inline-full` 或 `inline-1/2`、`inline-2/5` 等 `inline-<fraction>` 工具类，给元素设置基于百分比的 inline 尺寸：
...
使用 `inline-sm`、`inline-xl` 等工具类，按容器尺寸比例为元素设置固定的 inline 尺寸：
...
使用 `inline-screen` 工具类，让元素的 inline 尺寸撑满整个视口：
...
使用 `inline-auto` 工具类，在特定条件下（例如某个断点）移除元素已设置的 inline 尺寸：
