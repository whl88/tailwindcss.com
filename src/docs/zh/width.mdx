...
使用 `w-24`、`w-64` 等 `w-<number>` 工具类，按 spacing 比例为元素设置固定宽度：
...
使用 `w-full` 或 `w-1/2`、`w-2/5` 等 `w-<fraction>` 工具类，给元素设置基于百分比的宽度：
...
使用 `w-sm`、`w-xl` 等工具类，按容器尺寸比例为元素设置固定宽度：
...
使用 `w-auto` 工具类，在特定条件下（例如某个断点）移除元素已设置的宽度：
