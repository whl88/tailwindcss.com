...
export const description = "用于控制元素 3D 变换视距的工具类。";
...
使用 `perspective-normal`、`perspective-distant` 等工具类，控制 z 平面距离屏幕的远近：
...
使用 `perspective-none` 工具类，移除元素已有的 perspective 变换：
