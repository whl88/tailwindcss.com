...
export const description = "用于控制定位元素位置的工具类。";
...
使用 `top-0`、`bottom-4` 等 `top-<number>`、`right-<number>`、`bottom-<number>`、`left-<number>`、`inset-<number>` 工具类，设置[已定位元素](/docs/position)的水平或垂直位置：
...
如需使用负的 top/right/bottom/left 值，在类名前加上连字符即可将其转换为负值：
...
使用 `inset-s-0`、`inset-e-4` 等 `inset-s-<number>` 或 `inset-e-<number>` 工具类，设置 `inset-inline-start` 与 `inset-inline-end` 这两个[逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)——它们会根据文字方向映射到左侧或右侧：
