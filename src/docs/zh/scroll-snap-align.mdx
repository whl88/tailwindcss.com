...
export const description = "用于控制元素 scroll snap 对齐方式的工具类。";
...
使用 `snap-center` 工具类，让元素在 snap 容器中滚动时按中心位置吸附：
...
使用 `snap-start` 工具类，让元素在 snap 容器中滚动时按起始位置吸附：
...
使用 `snap-end` 工具类，让元素在 snap 容器中滚动时按结束位置吸附：
