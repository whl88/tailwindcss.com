...
export const description = "用于控制文本大小写转换的工具类。";
...
使用 `uppercase` 工具类，把元素中的文本全部转换为大写：
...
使用 `lowercase` 工具类，把元素中的文本全部转换为小写：
...
使用 `capitalize` 工具类，把元素中每个单词的首字母大写：
...
使用 `normal-case` 工具类，保留元素文本原本的大小写——常用于在不同断点处重置大小写转换：
