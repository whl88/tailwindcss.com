...
export const description = "用于在强制配色（forced colors）模式下控制是否参与适配的工具类。";
...
使用 `forced-color-adjust-none` 工具类，让元素不受强制配色模式（forced colors）的颜色覆盖。当强制使用受限调色板会降低可用性时，这非常有用。
...
你还可以使用 [forced colors 变体](/docs/hover-focus-and-other-states#forced-colors)，在用户启用了强制配色模式时按需追加样式。

### 恢复强制配色

使用 `forced-color-adjust-auto` 工具类，让元素遵循强制配色模式（forced colors）应用的颜色：
...
这适合在某些场景（例如更大屏幕下）撤销 `forced-color-adjust-none` 工具类。
