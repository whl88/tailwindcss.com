...
export const description = "用于设置元素最大宽度的工具类。";
...
使用 `max-w-24`、`max-w-64` 等 `max-w-<number>` 工具类，按 spacing 比例为元素设置固定的最大宽度：
...
使用 `max-w-full` 或 `max-w-1/2`、`max-w-2/5` 等 `max-w-<fraction>` 工具类，给元素设置基于百分比的最大宽度：
...
使用 `max-w-sm`、`max-w-xl` 等工具类，按容器尺寸比例为元素设置固定的最大宽度：
...
使用 `container` 工具类，把元素的最大宽度设为当前断点的 `min-width`。如果你更倾向于按一组固定屏幕尺寸来做设计，而不是适配完全流式的视口，这非常有用：
