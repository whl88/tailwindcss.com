...
export const description = "用于通过 transform 倾斜元素的工具类。";
...
使用 `skew-4`、`skew-10` 等 `skew-<number>` 工具类，让元素在两个轴向上同时倾斜：
...
使用 `-skew-4`、`-skew-10` 等 `-skew-<number>` 工具类，按反方向让元素在两个轴向上同时倾斜：
...
使用 `skew-x-4`、`-skew-x-10` 等 `skew-x-<number>` 工具类，让元素沿 x 轴倾斜：
...
使用 `skew-y-4`、`-skew-y-10` 等 `skew-y-<number>` 工具类，让元素沿 y 轴倾斜：
