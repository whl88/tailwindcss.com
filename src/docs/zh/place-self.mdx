...
export const description = "用于一次性控制单个 grid 子项横纵两轴对齐方式的工具类。";
...
使用 `place-self-auto`，让单个子项按容器 `place-items` 的取值对齐：
...
使用 `place-self-start`，让单个子项在两个轴向都对齐到起始端：
...
使用 `place-self-center`，让单个子项在两个轴向都居中：
...
使用 `place-self-end`，让单个子项在两个轴向都对齐到末端：
...
使用 `place-self-stretch`，让单个子项在两个轴向都拉伸：
