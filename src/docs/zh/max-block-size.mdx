...
export const description = "用于设置元素 block 方向最大尺寸的工具类。";
...
使用 `max-block-24`、`max-block-64` 等 `max-block-<number>` 工具类，按 spacing 比例为元素设置固定的最大 block 尺寸：
...
使用 `max-block-full` 或 `max-block-1/2`、`max-block-2/5` 等 `max-block-<fraction>` 工具类，给元素设置基于百分比的最大 block 尺寸：
