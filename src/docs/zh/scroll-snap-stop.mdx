...
export const description = "用于控制是否允许跳过 snap 停靠点的工具类。";
...
将 `snap-always` 工具类与 [snap-mandatory](/docs/scroll-snap-type#mandatory-scroll-snapping) 工具类配合使用，强制 snap 容器在用户滚到下一项之前必须停在某个元素上：
...
使用 `snap-normal` 工具类，允许 snap 容器跳过中间可能的吸附位置：
