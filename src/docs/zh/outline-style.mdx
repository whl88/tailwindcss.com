...
export const description = "用于控制元素轮廓（outline）样式的工具类。";
...
使用 `outline-solid`、`outline-dashed` 等工具类，设置元素 outline 的样式：
...
使用 `outline-hidden` 工具类，隐藏聚焦元素上浏览器默认的 outline，同时在强制配色模式下仍然保留 outline：
...
使用该工具类时，强烈建议你为 focus 状态自定义可访问的样式。

### 移除 outline

使用 `outline-none` 工具类，完全移除聚焦元素上浏览器默认的 outline：
