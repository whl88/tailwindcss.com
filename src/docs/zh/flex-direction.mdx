...
export const description = "用于控制 flex 子项排列方向的工具类。";
...
使用 `flex-row`，让 flex 子项沿文字方向横向排列：
...
使用 `flex-row-reverse`，让 flex 子项沿反向横向排列：
...
使用 `flex-col-reverse`，让 flex 子项沿反向纵向排列：
