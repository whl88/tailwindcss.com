...
export const description = "用于缩放元素的工具类。";
...
使用 `scale-75`、`scale-150` 等 `scale-<number>` 工具类，按原始尺寸的百分比缩放元素：
...
使用 `scale-x-75`、`-scale-x-150` 等 `scale-x-<number>` 工具类，按原始宽度的百分比沿 x 轴缩放元素：
...
使用 `scale-y-75`、`scale-y-150` 等 `scale-y-<number>` 工具类，按原始高度的百分比沿 y 轴缩放元素：
...
使用 `-scale-x-75`、`-scale-125` 等 `-scale-<number>`、`-scale-x-<number>` 或 `-scale-y-<number>` 工具类，按原始尺寸的百分比镜像并缩小元素：
