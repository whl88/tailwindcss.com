...
export const description = "用于控制元素 display 类型的工具类。";
...
使用 `inline`、`inline-block`、`block` 工具类，控制文本和元素的排版流向：
...
使用 `flow-root` 工具类，创建一个拥有自身[块级格式化上下文](https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Block_formatting_context)的块级元素：
...
使用 `inline-flex` 工具类，创建一个随文本流动的内联 flex 容器：
...
使用 `inline-grid` 工具类，创建一个内联的 grid 容器：
...
使用 `contents` 工具类，创建一个“幽灵”容器，其子元素表现得像父元素的直接子元素：
...
使用 `table`、`table-row`、`table-cell`、`table-caption`、`table-column`、`table-column-group`、`table-header-group`、`table-row-group`、`table-footer-group` 工具类，让元素表现得像对应的表格元素：
...
如果只想在视觉上隐藏元素但仍保留在文档布局中，请改用 [visibility](/docs/visibility#making-elements-invisible) 属性。

### 仅屏幕阅读器可见

使用 `sr-only`，在视觉上隐藏元素但仍对屏幕阅读器可见：
...
使用 `not-sr-only`，撤销 `sr-only`，让元素对普通用户和屏幕阅读器都可见：
