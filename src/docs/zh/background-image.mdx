...
export const description = "用于控制元素背景图的工具类。";
...
将 `bg-linear-to-r`、`bg-linear-<angle>` 等工具类与[颜色停靠点工具类](#setting-gradient-color-stops)配合使用，给元素添加线性渐变：
...
将 `bg-radial` 和 `bg-radial-[<position>]` 工具类与[颜色停靠点工具类](#setting-gradient-color-stops)配合使用，给元素添加径向渐变：
...
将 `bg-conic` 和 `bg-conic-<angle>` 工具类与[颜色停靠点工具类](#setting-gradient-color-stops)配合使用，给元素添加锥形渐变：
...
使用 `from-indigo-500`、`via-purple-500`、`to-pink-500` 等工具类，设置渐变各停靠点的颜色：
...
使用 `from-10%`、`via-30%`、`to-90%` 等工具类，更精确地控制渐变颜色停靠点的位置：
