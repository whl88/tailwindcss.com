...
export const description = "用于控制元素字号的工具类。";
...
使用 `text-sm`、`text-lg` 等工具类，设置元素的字号：
...
使用 `text-sm/6`、`text-lg/7` 等工具类，一次性设置元素的字号和行高：
...
你还可以为某个字号提供默认的 `line-height`、`letter-spacing` 和 `font-weight` 值：
