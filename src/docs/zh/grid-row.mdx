...
export const description = "用于控制元素跨越的网格行以及尺寸的工具类。";
...
使用 `row-span-2`、`row-span-4` 等 `row-span-<number>` 工具类，让元素跨越 _n_ 行：
...
使用 `row-start-2`、`row-end-3` 等 `row-start-<number>` 或 `row-end-<number>` 工具类，让元素从第 _n_ 条网格线开始或结束：
...
它们还可以与 `row-span-<number>` 工具类组合，跨越指定数量的行。
