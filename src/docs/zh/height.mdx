...
export const description = "用于设置元素高度的工具类。";
...
使用 `h-24`、`h-64` 等 `h-<number>` 工具类，按 spacing 比例为元素设置固定高度：
...
使用 `h-full` 或 `h-1/2`、`h-2/5` 等 `h-<fraction>` 工具类，给元素设置基于百分比的高度：
...
使用 `h-dvh` 工具类，让元素高度撑满整个视口（随浏览器 UI 展开/收起而变化）：
...
使用 `size-px`、`size-4`、`size-full` 等工具类，一次性设置元素的宽度和高度：
