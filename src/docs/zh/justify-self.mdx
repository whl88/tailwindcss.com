...
export const description = "用于控制单个 grid 子项在主轴（inline 轴）方向对齐方式的工具类。";
...
使用 `justify-self-auto` 工具类，让单个子项按网格 `justify-items` 的取值对齐：
...
使用 `justify-self-start` 工具类，让单个 grid 子项在 inline 轴方向对齐到起始端：
...
使用 `justify-self-center` 或 `justify-self-center-safe` 工具类，让单个 grid 子项在 inline 轴方向居中：
...
空间不足时，`justify-self-center-safe` 工具类会把子项对齐到容器的起始端，而不是末端。

### End 末端

使用 `justify-self-end` 或 `justify-self-end-safe` 工具类，让单个 grid 子项在 inline 轴方向对齐到末端：
...
空间不足时，`justify-self-end-safe` 工具类会把子项对齐到容器的起始端，而不是末端。

### Stretch 拉伸

使用 `justify-self-stretch` 工具类，让单个 grid 子项沿 inline 轴方向拉伸以填满网格区域：
