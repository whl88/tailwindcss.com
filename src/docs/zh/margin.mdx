...
export const description = "用于控制元素 margin 的工具类。";
...
使用 `m-4`、`m-8` 等 `m-<number>` 工具类，统一控制元素四周的 margin：
...
使用 `ml-2`、`mt-6` 等 `mt-<number>`、`mr-<number>`、`mb-<number>`、`ml-<number>` 工具类，单独控制元素某一侧的 margin：
...
使用 `mx-4`、`mx-8` 等 `mx-<number>` 工具类，控制元素的水平 margin：
...
使用 `my-4`、`my-8` 等 `my-<number>` 工具类，控制元素的垂直 margin：
...
如需使用负的 margin 值，在类名前加上连字符即可将其转换为负值：
...
使用 `ms-4`、`me-8` 等 `ms-<number>` 或 `me-<number>` 工具类，设置 `margin-inline-start` 与 `margin-inline-end` 这两个逻辑属性：
...
使用 `mbs-<number>` 与 `mbe-<number>` 工具类，设置 `margin-block-start` 与 `margin-block-end` 这两个逻辑属性——它们会根据书写模式映射到顶部或底部：
...
使用 `space-x-4`、`space-y-8` 等 `space-x-<number>` 或 `space-y-<number>` 工具类，控制相邻元素之间的间距：
...
如果元素是反向排列的（例如使用了 `flex-row-reverse` 或 `flex-col-reverse`），请使用 `space-x-reverse` 或 `space-y-reverse` 工具类，确保间距被加到每个元素正确的一侧：
...
另外，`space-*` 工具类并未针对与[分隔线工具类](/docs/border-width#between-children)同时使用做设计。这种场景建议改为给子元素添加 margin/padding 工具类。
