...
export const description = "用于控制元素如何与背景混合的工具类。";
...
使用 `mix-blend-overlay`、`mix-blend-soft-light` 等工具类，控制元素的内容与背景如何与同一层叠上下文中的其他内容混合：
...
在父元素上使用 `isolate` 工具类，创建一个新的层叠上下文，避免与其后方的内容混合：
