...
export const description = "在 Tailwind CSS 项目中使用并自定义调色板。";

Tailwind CSS 内置了一套庞大、精美的调色板，由资深设计师精心打造，适用于各种不同的设计风格。
...
默认调色板中的每种颜色都包含 11 个明度档位，50 最浅，950 最深：
...
整套调色板可在所有与颜色相关的工具类中使用，包括 [background color](/docs/background-color)、[border color](/docs/border-color)、[fill](/docs/fill)、[caret color](/docs/caret-color) 等许多工具。

## 使用颜色

### 使用颜色工具类

使用 `bg-white`、`border-pink-300`、`text-gray-950` 等颜色工具类，为设计中的元素设置不同的颜色属性：
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        设置元素的<a href="/docs/background-color">背景色</a>
      </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>
        设置<a href="/docs/box-shadow#setting-the-shadow-color">盒阴影</a>的颜色
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        设置<a href="/docs/box-shadow#setting-the-inset-shadow-color">内嵌盒阴影</a>的颜色
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        设置<a href="/docs/box-shadow#setting-the-ring-color">ring 阴影</a>的颜色
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        设置<a href="/docs/box-shadow#setting-the-inset-ring-color">内嵌 ring 阴影</a>的颜色
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        设置表单控件的<a href="/docs/accent-color">主题色</a>
      </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>
        设置 SVG 元素的<a href="/docs/fill">填充色</a>
      </td>
    </tr>
    <tr>
      <td>
...
你可以使用 `bg-black/75` 这样的语法调整颜色的不透明度，其中 `75` 表示把颜色的 alpha 通道设为 75%：
...
使用 `dark` 变体编写如 `dark:bg-gray-800` 这样的类，仅在深色模式激活时应用对应颜色：
...
更多关于深色模式样式的内容请参阅[深色模式文档](/docs/dark-mode)。

### 在 CSS 中引用

所有颜色都以 CSS 变量的形式暴露在 `--color-*` 命名空间下，因此你可以在 CSS 中通过 `--color-blue-500`、`--color-pink-700` 这类变量引用它们：
...
在 CSS 中以变量形式引用颜色时若想快速调整不透明度，Tailwind 提供了一个特殊的 <code>--alpha()</code> 函数：
...
使用 `@theme` 在 `--color-*` 主题命名空间下为项目添加自定义颜色：
...
这样在你的项目中除了默认颜色外，还能使用 `bg-midnight`、`text-tahiti`、`fill-bermuda` 等工具类。

更多关于主题变量的内容请参阅[主题变量文档](/docs/theme)。
...
把某个颜色的主题命名空间设为 `initial`，即可禁用该默认颜色：
...
对于你不打算使用的颜色，这能从输出中移除对应的 CSS 变量。

### 使用自定义调色板

使用 `--color-*: initial` 完全禁用所有默认颜色，并定义你自己的调色板：
...
更多内容请参阅主题文档中的[引用其他变量](/docs/theme#referencing-other-variables)章节。

## 默认调色板参考

以下是默认颜色及其取值的完整列表，供参考：
...
如果你想以不同的名字复用这些刻度（例如把 `--color-gray-*` 重新定义为使用 `--color-slate-*` 的刻度），这一参考会很有用。
