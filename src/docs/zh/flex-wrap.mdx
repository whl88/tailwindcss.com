...
export const description = "用于控制 flex 子项是否换行的工具类。";
...
使用 `flex-nowrap`，禁止 flex 子项换行，必要时不可伸缩的子项会溢出容器：
...
使用 `flex-wrap-reverse`，让 flex 子项按相反方向换行：
