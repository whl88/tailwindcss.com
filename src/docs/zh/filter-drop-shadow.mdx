...
export const description = "用于给元素添加投影滤镜的工具类。";
...
使用 `drop-shadow-sm`、`drop-shadow-xl` 等工具类，为元素添加投影：
...
它适合给文本、SVG 等不规则形状添加阴影。普通元素的阴影通常更适合用 [box shadow](/docs/box-shadow)。

### 修改不透明度

通过 opacity 修饰符调整投影的不透明度：
...
使用 `drop-shadow-indigo-500`、`drop-shadow-cyan-500/50` 等工具类，修改投影的颜色：
...
使用 `drop-shadow-none` 工具类，移除元素已有的投影：
