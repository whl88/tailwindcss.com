...
export const description = "用于设置元素最小宽度的工具类。";
...
使用 `min-w-24`、`min-w-64` 等 `min-w-<number>` 工具类，按 spacing 比例为元素设置固定的最小宽度：
...
使用 `min-w-full` 或 `min-w-1/2`、`min-w-2/5` 等 `min-w-<fraction>` 工具类，给元素设置基于百分比的最小宽度：
...
使用 `min-w-sm`、`min-w-xl` 等工具类，按容器尺寸比例为元素设置固定的最小宽度：
