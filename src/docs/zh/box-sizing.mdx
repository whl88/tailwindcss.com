...
export const description = "用于控制浏览器如何计算元素总尺寸的工具类。";
...
使用 `box-border` 工具类，把元素的 `box-sizing` 设为 `border-box`，让浏览器在计算指定的宽高时把边框和 padding 一并包含进去。

也就是说，一个 100px &times; 100px 的元素，加上 2px 边框和四周 4px 的 padding，最终仍然渲染为 100px &times; 100px，内部内容区为 88px &times; 88px：
...
使用 `box-content` 工具类，把元素的 `box-sizing` 设为 `content-box`，让浏览器在指定宽高之外再加上边框和 padding。

也就是说，一个 100px &times; 100px 的元素，加上 2px 边框和四周 4px 的 padding，实际会渲染为 112px &times; 112px，内部内容区为 100px &times; 100px：
