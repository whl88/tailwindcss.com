...
export const description = "用于控制 grid / flex 子项之间间距的工具类。";
...
使用 `gap-2`、`gap-4` 等 `gap-<number>` 工具类，同时调整 grid / flex 布局中行与列的间距：
...
使用 `gap-x-8`、`gap-y-4` 等 `gap-x-<number>` 或 `gap-y-<number>` 工具类，分别调整列与行的间距：
