...
使用 `border-rose-500`、`border-lime-100` 等工具类，控制元素的边框颜色：
...
使用 `border-t-indigo-500`、`border-r-lime-100` 等工具类，单独设置元素某一侧的边框颜色：
...
使用 `border-x-indigo-500`、`border-y-lime-100` 等工具类，同时设置元素相对两侧的边框颜色：
...
使用 `border-s-indigo-500`、`border-e-lime-100` 等工具类，设置 `border-inline-start-color` 和 `border-inline-end-color` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，它们会根据文字方向映射到左侧或右侧的边框：
...
使用 `border-bs-*` 和 `border-be-*` 工具类，设置 `border-block-start-color` 和 `border-block-end-color` 逻辑属性，它们会根据书写模式映射到顶部或底部边框：
...
使用 `divide-indigo-500`、`divide-lime-100` 等工具类，控制相邻子元素之间分隔线的颜色：
