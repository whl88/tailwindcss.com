...
export const description = "用于控制元素在文档流中的定位方式的工具类。";
...
使用 `relative` 工具类，让元素按文档正常流定位（同时建立相对定位上下文）：
...
使用 `absolute` 工具类，把元素从文档正常流中*取出*定位，相邻元素会当它不存在：
...
使用 `sticky` 工具类，让元素先按 `relative` 定位，越过指定阈值后切换为 `fixed`，直到父元素离开屏幕：
