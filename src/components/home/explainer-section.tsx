...
export default async function ExplainerSection() {
  const t = await getTranslations("Home.explainer");
  ...
  return (
    <div className="relative max-w-full">
      ...
      <GridContainer className="2xl:before:hidden 2xl:after:hidden">
        <CategoryHeader className="text-sky-500 dark:text-sky-400">{t("category")}</CategoryHeader>
      </GridContainer>
                ...
                <EditorAnimation timeline={timeline}>
                  ...
                  <div className="flex w-full flex-col gap-1 lg:h-132.5 lg:flex-row">
                    <div className="flex flex-col gap-1 lg:w-1/2 xl:w-5/8">
                      <Panel className="min-h-0 flex-1 max-lg:max-h-76" aria-label={t("htmlPanelAria")}>
                        <Tabs tabBarAriaLabel={t("tabBarAria")}>
                        ...
                        <HtmlFile className="with-line-numbers" editorAriaLabel={t("editorHtmlAria")} />
                      </Panel>
                      <Panel className="h-32 shrink-0 lg:h-46" aria-label={t("terminalPanelAria")}>
                        <Tabs tabBarAriaLabel={t("tabBarAria")}>
                          <Tab selected>{t("tabTerminal")}</Tab>
                        </Tabs>
                        <Terminal />
                      </Panel>
                    </div>
                    <div className="flex h-66 flex-col gap-2.5 lg:h-auto lg:w-1/2 xl:w-3/8">
                      <Panel className="h-full" aria-label={t("cssPanelAria")}>
                        <Tabs tabBarAriaLabel={t("tabBarAria")}>
                          <Tab selected>build.css</Tab>
                        </Tabs>
                        <BuiltCss className="with-line-numbers" timeline={timeline} editorAriaLabel={t("editorCssAria")} />
                      </Panel>
                    </div>
                  </div>
                </EditorAnimation>
...
function HtmlFile({ className, editorAriaLabel }: { className?: string; editorAriaLabel: string }) {
  return (
    <HighlightedCode
...
function Tabs({ children, tabBarAriaLabel }: { children: React.ReactNode; tabBarAriaLabel: string }) {
  return (
    <div
